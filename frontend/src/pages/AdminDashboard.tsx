...
const AdminDashboard: React.FC = () => {
  ...
  const deleteUser = (id: number) => {
    const deleted = users.find((u) => u.id === id);
    setUsers((prev) => prev.filter((u) => u.id !== id));
    if (deleted) addLog(`Deleted user: ${deleted.name}`);
  };
  ...
  const addFile = () => {
    ...
    setFiles((prev) => [...prev, newFile]);
    addLog(`Uploaded file: ${newFile.name}`);
  };

  const deleteFile = (id: number) => {
    const deleted = files.find((f) => f.id === id);
    setFiles((prev) => prev.filter((f) => f.id !== id));
    if (deleted) addLog(`Deleted file: ${deleted.name}`);
  };

  // Simulated extra actions
  const simulateLogin = () => addLog("User logged in: admin@example.com");
  const simulateLogout = () => addLog("User logged out");
  const simulateFilter = () => addLog("Applied filter on dataset (120 → 45 rows)");
  const simulateExport = () => addLog("Exported filtered dataset to Excel");
        ...
        {activeTab === "overview" && (
          <>
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={simulateLogin} className="px-4 py-2 text-white bg-green-600 rounded-lg">
                Simulate Login
              </button>
              <button onClick={simulateLogout} className="px-4 py-2 text-white bg-gray-600 rounded-lg">
                Simulate Logout
              </button>
              <button onClick={simulateFilter} className="px-4 py-2 text-white bg-blue-600 rounded-lg">
                Simulate Filter
              </button>
              <button onClick={simulateExport} className="px-4 py-2 text-white bg-purple-600 rounded-lg">
                Simulate Export
              </button>
            </div>
          </>
