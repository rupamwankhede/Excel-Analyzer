...
import { Users, FileSpreadsheet, Clock } from "lucide-react";
import type { ExcelData } from "../App";
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  return (
    <div className="min-h-screen p-6 bg-gray-100 dark:bg-gray-900 dark:text-gray-100">
      ...
      {excelData && (
        <div className="p-6 mb-8 bg-white shadow-md dark:bg-gray-800 rounded-xl">
          <h2 className="mb-4 text-xl font-bold">Uploaded File</h2>
          <p className="text-lg font-semibold">{excelData.filename}</p>
          <p className="text-sm text-gray-600 dark:text-gray-400">
            Rows: {excelData.stats.totalRows.toLocaleString()}, Columns:{" "}
            {excelData.stats.totalColumns}
          </p>
          <p className="text-sm text-gray-600 dark:text-gray-400">
            Sheets: {excelData.sheetNames.join(", ")}
          </p>
        </div>
      )}
        ...
        {logs.length === 0 ? (
          ...
          <ul className="space-y-2 overflow-y-auto max-h-80">
            {logs.map((log, i) => (
              <li
                key={i}
                className="p-3 text-sm bg-gray-100 rounded-md dark:bg-gray-700"
              >
                {log}
              </li>
            ))}
...
const StatCard = ({
  ...
  <div className="flex flex-col p-6 bg-white shadow-md dark:bg-gray-800 rounded-xl">
    <div className="mb-2 text-gray-600 dark:text-gray-300">{icon}</div>
    <p className="text-sm">{label}</p>
    <p className="text-2xl font-bold">{value}</p>
  </div>
);
